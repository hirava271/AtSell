...
<c:forEach items="${sessionScope.edit_subcat_session }" var="i">
<form class="form-horizontal" method="post" action="<%=request.getContextPath()%>/SubcategoryServlet?flag=save&id=${i.subcategory_id}">
...
<c:if test="${j.category_id eq i.catVO.category_id }">
<option value="${i.catVO.category_id }" selected="selected">${i.catVO.category_name }</option>
</c:if> 
<c:if test="${j.category_id ne i.catVO.category_id }">
...
<div class="col-sm-10">
<input type="text" class="form-control" name="add_subcategory_name" value="${i.subcategory_name}">
</div>
</div>
...
<div class="col-sm-10">
<textarea class="form-control" rows="3" name="add_subcategory_description">${i.subcategory_description}</textarea>
</div>
</div>
