	...
	<div class="content-wrap">
		...
		<div class="wrapper">
			...
			<div class="panel-body">
				...
				<form action="<%=request.getContextPath()%>/DManageShippingServlet?flag=change_status" method="post">
					<table
						...
						<tbody>
							...
							<c:forEach items="${sessionScope.manage_shipping}" var="i"
								varStatus="ii">
								<c:set var="mst" value="${i[0]}"></c:set>
								<c:set var="dtl" value="${i[1]}"></c:set>
								<tr>
									<td><input type="checkbox" name="checkbox_order" value="${mst.cart_mst_id}">
									<td>${ii.count}</td>
									<td>${mst.cart_mst_id }</td>
									<td>${mst.date}</td>
									<td>${mst.buyer_user_id.first_name}
										&nbsp;${mst.buyer_user_id.last_name}</td>
									<td>${mst.shipping_address}</td>
									<td>${mst.countryVO.country_name}</td>
									<td>${mst.stateVO.state_name}</td>
									<td>${mst.cityVO.city_name}</td>
									<td>${mst.contact_no }
					...
					<button type="submit" value="Delivered" name="b" style="height: 25px; width:150px; margin-left:475px">Delivered</button>
					</form>
				</div>
			</div>
			</section>
