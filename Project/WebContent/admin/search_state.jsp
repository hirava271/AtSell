...
<c:forEach items="${sessionScope.state}" var="i" varStatus="ii">
<tbody>
	<tr>
		<td> ${ii.count}</td>
		<td> ${i.state_name}</td>
		<td> ${i.state_description}</td>
		<td> <a href="<%=request.getContextPath() %>/StateServlet?flag=delete&id=${i.state_id}">Delete</a> /
			 <a href="<%=request.getContextPath() %>/StateServlet?flag=edit&id=${i.state_id}">Edit</a></td>
	</tr>
</tbody>
</c:forEach>
</table>
