...
<c:forEach items="${sessionScope.search_package}" var="i" varStatus="ii">
<tbody>
<tr> 
	<td>
		${ii.count}
	...
	<td>
		<c:if test="${i.isActive eq 1}"><c:out value="Active"></c:out></c:if>
		<c:if test="${i.isActive ne 1}"><c:out value="InActive"></c:out></c:if>
	</td>
	<td><a href="<%=request.getContextPath()%>/PackageServlet?flag=delete&id=${i.package_id}">Delete</a>/
	    <a href="<%=request.getContextPath()%>/PackageServlet?flag=edit&id=${i.package_id}">Edit</a></td>
</tr>
</tbody>
</c:forEach>
